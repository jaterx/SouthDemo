#version 400

// Attributes passed automatically by OGRE
in vec3 vertex;
in vec3 normal;
in vec4 colour;

// Attributes passed with the material file
uniform mat4 world_mat;
uniform mat4 view_mat;
uniform mat4 normal_mat;
uniform vec3 up_vec;
uniform float timer;

// Attributes forwarded to the geometry shader
out vec4 particle_colour;
out float particle_id;

// Define some useful constants
const float pi = 3.1415926536;
const float pi_over_two = 1.5707963268;
const float two_pi = 2.0*pi;

void main()
{
    // Define particle id
	particle_id = colour.r; // Derived from the particle colour. We use the id to keep track of particles

	// Define time in a cyclic manner
    float phase = two_pi*particle_id; // Start the sin wave later depending on the particle_id
	float param = timer / 5.0 + phase; // The constant that divides "timer" adjusts the "speed" of the exhaust
	float rem = mod(param, pi_over_two); // Use the remainder of dividing by pi/2 so that we are always in the range [0..pi/2] where sin() gives values in [0..1]
	float circtime = sin(rem); // Get time value in [0..1], according to a sinusoidal wave
	                                
	// Set up parameters of the particle motion
    float t = abs(circtime)*(abs(normal.y*3)); // Our time parameter
	//increase the acceleration in order to extend the tail of the exhaust effect to make it more like rocket exhaust
    float accel = 0.2; // An acceleration applied to the particles coming from some attraction force
	//increase the speed in order to extend the frequency of the effect to make it more like rocket exhaust
    float slow = 0.2; // Allows us to slow down the motion, control its speed

	// First, work in local model coordinates (do not apply any transformation)
	vec3 position = vertex;
	position -= 0.001*slow*accel*t*t; // Particle moves down //*up_vec, change wind direction
	
	// Define output position but do not apply the projection matrix yet
	//offset the y position a little in order to visualize the long exhaust tail
	gl_Position = view_mat * world_mat * vec4(position.x, position.y+150.0, position.z, 1.0);
	
	// Define amount of blending depending on the cyclic time    
	float alpha = 1.0 - circtime*circtime;
	particle_colour = vec4(1.0, 1.0, 1.0, alpha);
}