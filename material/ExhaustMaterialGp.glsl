...
void main(void){
    ...
    int fid = int(floor(particle_id[0] * 4.0)); // 0-3 used to pick sector from exhaust 2x2 drawing
    for (int i = 0; i < 4; i++){
		gl_Position = projection_mat * v[i];
		//offset the texture coordinate by multiply 0.1 in order to create steam effect (hot air around the exhaust is less densier than normal, which will cause the air become unstable (blurring or shaking, as visualization))
